...
const PosSystem = () => {
    ...
    useEffect(() => {
        // Fetch full stock details for POS
        employeeService.getStock().then(setProducts).catch(console.error);
    }, []);
    ...
    const handleCreateOrder = async () => {
        ...
        // POS orders: Usually cash, completed instantly or pending payment
        // We create it as PENDING first to ensure stock deduction happens correctly via backend transaction
        const now = new Date();
        const formattedTime = `${now.getHours().toString().padStart(2, '0')}:${now.getMinutes().toString().padStart(2, '0')}`;

        const orderPayload = {
            cus_id: null, // Walk-in customer
