...
import { ShoppingBag, LogOut, User as UserIcon, UserPlus, Search, Menu as MenuIcon, X, ChevronRight, Minus, Plus, Trash2, MapPin, ChevronDown, Facebook, Instagram, Linkedin, Phone, Mail, Clock } from 'lucide-react';

export const Navbar = () => {
  const { currentUser, cart, logout, setIsCartOpen } = useApp();
  ...
  const handleProductSearch = (e: React.FormEvent) => {
    e.preventDefault();
    // In a real app, this would filter via URL params.
    navigate(`/menu?search=${encodeURIComponent(productSearch)}`);
  };

  const handleOrderSearch = (e: React.FormEvent) => {
    e.preventDefault();
    alert(`Searching for Order ID: ${orderSearch} (Feature coming soon)`);
  };
              ...
              {!currentUser ? (
                ...
                <div className="flex items-center gap-4">
                   <div className="text-right hidden sm:block">
                     ...
                     <p className="text-sm font-bold text-tlj-green">{currentUser.fullName.split(' ')[0]}</p>
                   </div>
      ...
      <nav className={`bg-tlj-green text-white shadow-md sticky top-0 z-40 transition-all duration-300 ${scrolled ? 'py-2' : 'py-3'}`}>
         <div className="max-w-7xl mx-auto px-4 flex items-center justify-between md:justify-center">
            ...
            <div className="hidden md:flex items-center space-x-12">
              ...
              <div className="relative group">
                ...
                <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-0 pt-2 w-48 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                   <div className="bg-white rounded-lg shadow-xl overflow-hidden border border-gray-100 flex flex-col">
                      <Link to="/menu?category=Bread" className="px-6 py-3 text-sm text-tlj-charcoal hover:bg-tlj-cream hover:text-tlj-green hover:font-bold transition-colors text-left font-serif border-b border-gray-50">
                        Bread
                      </Link>
                      <Link to="/menu?category=Cakes" className="px-6 py-3 text-sm text-tlj-charcoal hover:bg-tlj-cream hover:text-tlj-green hover:font-bold transition-colors text-left font-serif border-b border-gray-50">
                        Cakes
                      </Link>
                      <Link to="/menu?category=Coffee" className="px-6 py-3 text-sm text-tlj-charcoal hover:bg-tlj-cream hover:text-tlj-green hover:font-bold transition-colors text-left font-serif border-b border-gray-50">
                        Coffee
                      </Link>
                      <Link to="/menu?category=Milk" className="px-6 py-3 text-sm text-tlj-charcoal hover:bg-tlj-cream hover:text-tlj-green hover:font-bold transition-colors text-left font-serif">
                        Milk
                      </Link>
                   </div>
                </div>
              ...
              <Link to="/policy" className={`uppercase text-sm tracking-widest ${isActive('/policy')}`}>Policy</Link>
            ...
            {/* Mobile Cart Placeholder (Right aligned on mobile) */}
        ...
        <div className="fixed inset-0 z-50 bg-tlj-green/95 backdrop-blur-md flex flex-col justify-center items-center text-white transition-opacity duration-300">
           ...
           <nav className="flex flex-col space-y-6 text-center w-full px-8">
             ...
             {currentUser ? (
               ...
               <Link to="/login" onClick={() => setMobileMenuOpen(false)} className="text-2xl font-serif">Login</Link>
             )}
           </nav>
        </div>
      )}
...
const CartDrawer = () => {
  const { isCartOpen, setIsCartOpen, cart, removeFromCart, updateCartQuantity } = useApp();
  const navigate = useNavigate();
            ...
            {cart.length === 0 ? (
              ...
              cart.map((item) => (
                <div key={item.id} className="flex gap-4 group">
                  <div className="w-20 h-20 bg-white rounded-md overflow-hidden flex-shrink-0 shadow-sm">
                    <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
                  </div>
                  <div className="flex-1 flex flex-col justify-between">
                    <div>
                      <h3 className="font-serif text-tlj-charcoal font-medium leading-tight mb-1">{item.name}</h3>
...
export const Footer = () => (
  <footer className="bg-tlj-green text-white pt-16 pb-8 border-t border-white/10">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      {/* Upper Content - Horizontal Layout */}
      <div className="grid grid-cols-1 md:grid-cols-12 gap-12 lg:gap-8 mb-16">
        ...
        {/* Column 2: BAKERY (Spans 2 columns) */}
        ...
        {/* Column 3: BEVERAGES (Spans 2 columns) */}
        <div className="md:col-span-2 lg:col-span-2">
        ...
        {/* Column 4: COMPANY (Spans 4 columns) */}
