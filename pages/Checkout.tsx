...
const Checkout = () => {
  ...
  // Form States
  const [customer, setCustomer] = useState({ name: currentUser?.fullName || '', phone: currentUser?.phone || '', note: '' });
  const [city, setCity] = useState("Ha Noi");
  const [district, setDistrict] = useState("Hai Ba Trung");
  const [street, setStreet] = useState("");
  const [payment, setPayment] = useState("cod"); // cod | bank
        ...
        <form onSubmit={handlePlaceOrder} className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1">
                            ...
                            <input required type="text" value={customer.name} onChange={e => setCustomer({...customer, name: e.target.value})} className="w-full p-2 border border-gray-200 rounded outline-none focus:border-tlj-green" />
                        </div>
                        <div className="space-y-1">
                            <label className="text-xs font-bold uppercase text-gray-400">Phone</label>
                            <input required type="text" value={customer.phone} onChange={e => setCustomer({...customer, phone: e.target.value})} className="w-full p-2 border border-gray-200 rounded outline-none focus:border-tlj-green" />
                        </div>
                        <div className="md:col-span-2 space-y-1">
                            <label className="text-xs font-bold uppercase text-gray-400">Note</label>
                            <textarea rows={2} value={customer.note} onChange={e => setCustomer({...customer, note: e.target.value})} className="w-full p-2 border border-gray-200 rounded outline-none focus:border-tlj-green" placeholder="Ex: Less sugar, deliver to lobby..."></textarea>
                        </div>
                    </div>
                </div>
            ...
            <div className="lg:col-span-5">
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 sticky top-24">
                    ...
                    <button type="submit" className="w-full py-4 bg-tlj-green text-white font-bold rounded-xl hover:bg-tlj-charcoal transition-all shadow-lg hover:shadow-xl">
                        {payment === 'bank' && !showQR ? 'Proceed to Payment' : 'Confirm Order'}
                    </button>
                </div>
            </div>
        </form>
      ...
      {showQR && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
             <div className="bg-white rounded-2xl p-6 max-w-sm w-full text-center relative">
                 ...
                 <button onClick={handlePlaceOrder} className="w-full py-3 bg-tlj-green text-white font-bold rounded-xl hover:bg-tlj-charcoal">I have paid</button>
             </div>
        </div>
      )}
    </div>
